...
int nextRun(Process* process, int size) {
    ...
    for (int i = 0; i < size; i++) {
        if (process[i].processID != -1) {
            if (highPriority.arrivalTime > process[i].arrivalTime) {
                highPriority = process[i];
                index = i;
            }
            else if (highPriority.arrivalTime == process[i].arrivalTime && highPriority.processID > process[i].processID) {
                highPriority = process[i];
                index = i;
            }
        }
