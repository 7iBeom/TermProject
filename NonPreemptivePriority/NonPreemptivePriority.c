...
void Shift(Process process[], int size) {
    ...
    for (int i = 0; i <= size; i++) {
        int j = 1;
        if (process[i].processID != -1 && process[i-j].processID == -1) {
            temp = process[i-j];
...
int nextRun(Process* process, int size) {
    ...
    for (int i = 0; i < size; i++) {
        if (process[i].processID != -1) {
            ...
            else if (highPriority.priority == process[i].priority && highPriority.arrivalTime == process[i].arrivalTime && highPriority.processID > process[i].processID ) {
                highPriority = process[i];
                index = i;
            }
        }
